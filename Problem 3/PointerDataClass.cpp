...
pointerDataClass::pointerDataClass(pointerDataClass &data) {
	
	// Initialize the max array size to be the size of array data
	maxSize = data.maxSize;
	
	// Initialize the array pointer to the created array with the desired size
	p = new int [maxSize];
	
	// For each element from data copy it into array p
	for (int i = 0; i < maxSize; i++) {
